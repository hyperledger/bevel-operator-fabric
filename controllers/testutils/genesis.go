...
func memberToOrg(member PeerOrganization) (*genesisconfig.Organization, error) {
	...
	err = os.WriteFile(path.Join(caCertsPath, "cacert.pem"), serverRootCert, os.ModePerm)
	if err != nil {
		return nil, err
	}
	err = os.WriteFile(path.Join(tlsCaCertsPath, "tlscacert.pem"), serverTlsCertPem, os.ModePerm)
	if err != nil {
		return nil, err
	}
	configNodeOU := `
...
`
	err = os.WriteFile(path.Join(mspDir, "config.yaml"), []byte(configNodeOU), os.ModePerm)
	if err != nil {
		return nil, err
	}
...
func memberToOrgUpdate(member PeerOrganization) (*genesisconfig2.Organization, error) {
	...
	err = os.WriteFile(path.Join(caCertsPath, "cacert.pem"), rootCertPem, os.ModePerm)
	if err != nil {
		return nil, err
	}
	err = os.WriteFile(path.Join(tlsCaCertsPath, "tlscacert.pem"), serverTlsCertPem, os.ModePerm)
...
`
	...
	log.Printf("MSPDIR=%s", mspDir)
	var anchorPeers []*genesisconfig2.AnchorPeer
	for _, peer := range member.Peers {
...
func AddConsortiumToConfig(channelConfig *cb.Config, request AddConsortiumRequest) (*cb.Config, error) {
	...
	var peerOrgs []*genesisconfig2.Organization
...
func GetProfileConfig(
	...
	for _, org := range ordOrgs {
		...
		for _, node := range org.Nodes {
			...
			err = os.WriteFile(clientCertFile.Name(), clientTlsCertPem, 644)
			if err != nil {
				return nil, err
			}
			...
			err = os.WriteFile(serverCertFile.Name(), serverTlsCertPem, 644)
			if err != nil {
				return nil, err
			}
		...
		err = os.WriteFile(path.Join(caCertsPath, "cacert.pem"), serverRootSignCertPem, os.ModePerm)
		if err != nil {
			return nil, err
		}
		err = os.WriteFile(path.Join(tlsCaCertsPath, "tlscacert.pem"), serverRootTlsCertPem, os.ModePerm)
		if err != nil {
			return nil, err
		}
		configNodeOU := `
NodeOUs:
  ...
  OrdererOUIdentifier:
    ...
    OrganizationalUnitIdentifier: orderer
`
		err = os.WriteFile(path.Join(ordererOrgMspDir, "config.yaml"), []byte(configNodeOU), os.ModePerm)
		if err != nil {
			return nil, err
		}
...
func GetChannelProfileConfig(
	...
	err = os.WriteFile(path.Join(caCertsPath, "cacert.pem"), serverRootCertPem, os.ModePerm)
	if err != nil {
		return nil, err
	}
	err = os.WriteFile(path.Join(tlsCaCertsPath, "tlscacert.pem"), serverRootTlsCertPem, os.ModePerm)
